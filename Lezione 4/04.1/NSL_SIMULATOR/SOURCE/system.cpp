  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4.0*sqrt(_temp)/(double)_n_bins_v; // TO BE FIXED IN EXERCISE 4, ORIGINALLI 4* MODIFIED TO 8*
        _measure_pofv = true;
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    for(int i=0; i<_npart; i++){
      ...
      _particle(i).setposition(0, this->pbc(0.5*_side(0)*x, 0)); //PER IL 04.2 INIZIARE CON 0.5* IN ONGNI SIDE COME A SINISTRA, TOGLIERE SE ITERO O NEL 04.3
      _particle(i).setposition(1, this->pbc(0.5*_side(1)*y, 1)); //PER 04.3 METTER SETPOSITIONOLD SENZA 0.5*
