...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    /*double _initial_velocity = sqrt(3.0*_temp); //added on ex4 for delta distribution !!!!segni invertiti!!!!
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    for(int i=0; i<_npart; i++){
      ...
      _particle(i).setposition(0, this->pbc(_side(0)*x, 0)); //PER IL 04.2 INIZIARE CON 0.5* IN ONGNI SIDE COME A SINISTRA, TOGLIERE SE ITERO O NEL 04.3
